...
**① 知识过期（时效性）** — 训练数据收集到什么时候，模型就只知道那个时间点之前的事。你问"今年发布的某款产品有哪些新功能"，如果训练数据截止在发布日期之前，它不知道，但它不会说"我不知道"，而是会用历史规律"推测"出一个听起来合理但可能是错的答案。

**② 知识空白（私有数据）** — 时效性说的是"旧了"，私有知识说的是"压根没有"。公司内部文档、产品手册、客服知识库、合同模板，这些东西根本不会出现在公开训练数据里。你让 LLM 回答"我们产品的退款政策是什么"，它不可能知道，如果它回答了，那一定是在编。

**③ 幻觉（知识缺失的副产品）** — 很多人以为幻觉是 LLM 的一个独立 bug，需要单独治理。其实不是。幻觉是知识缺失的副产品——LLM 的核心机制是"预测下一个词"，它没有内置"我不确定就停下来"的开关。当参数里的知识不够用时，它只能硬着头皮往下生成，把相关的、不相关的知识拼凑出一个答案。

> 💡 关键洞察：幻觉不是独立问题，而是知识缺失的副产品。RAG 解决的是根源（知识不够），不是单独在治幻觉。知识过期会导致幻觉，知识空白也会导致幻觉，根源都是同一件事：模型参数里没有对应的知识。
...
<details>
<summary>🔻 学习批注 | 从零训练 vs 微调、标注数据与 GPU（Q16~Q19）💬</summary>
<div style="color: #2e7d32; font-size: 0.85em;">

**Q16：从零训练一个模型和微调一个已有模型，有什么本质区别？**

本质上做的事情一样——都是用数据调整模型的权重参数。区别在于起点不同：

| | 从零训练（Pre-training） | 微调（Fine-tuning） |
|:---|:---|:---|
| 参数起点 | 随机数（啥也不懂） | 已训练好的权重 |
| 数据量 | TB 级（海量互联网文本） | MB~GB 级（几千到几万条） |
| 学什么 | 语言规律、世界知识 | 特定任务、输出风格 |
| 成本 | 千万级美元 | 百~千美元 |
| 时间 | 几周到几个月 | 几小时到几天 |
| 谁在做 | OpenAI、Meta、阿里等大厂 | 任何有 GPU 的团队 |

🎓 比喻：从零训练是"把婴儿从零养大"，微调是"给大学毕业生做岗前培训"。对绝大多数应用场景，拿现成的基础模型做微调就够了，没人需要从零训练。

**Q17：标注数据是什么？**

就是"带标准答案的练习题"。原始数据没有答案，标注就是人工给每条数据加上正确答案：

```
未标注：  "你们的退货政策是什么？"
标注后：  问："你们的退货政策是什么？"
         答："7天内无理由退货，需保持商品完好，联系客服即可办理。"
```

微调时模型就是在"刷题"——看问题、对答案、答错了调参数、反复练。标注数据贵就贵在需要懂业务的人一条一条写答案，几千条高质量数据可能要几个人标几周。

**Q18：为什么训练需要 GPU？**

训练的本质是大量的矩阵乘法。CPU 也能算，但太慢：

```
CPU：一个很聪明的人，一次做一道题
GPU：一万个普通人，每人同时做一道简单题，一起交卷
```

矩阵乘法恰好是"大量简单的乘法和加法"，GPU 天生擅长这种大规模并行计算。调整 90 亿个参数，CPU 要几个月，GPU 几小时到几天。

标注数据和 GPU 的关系：标注数据是"题目"，GPU 是"做题的机器"，缺一不可。没有标注数据，GPU 再快也没题可刷；没有 GPU，有题也刷不动。

**Q19：所以训练需要大量人力？业界怎么应对？**

是的，尤其标注数据环节。但业界的趋势是能不训练就不训练：

```
优先级1：直接用现成模型 + 好的 Prompt     → 零人力
优先级2：RAG（加知识库）                  → 少量人力整理文档
优先级3：微调                            → 需要标注团队
优先级4：从零训练                         → 需要大团队 + 巨额投入
```

大多数应用场景走到优先级 2（RAG）就够了。这也是为什么企业落地 AI 问答系统，RAG 是首选方案。

**Q20：LLM 发布后的文件存的全是向量数据吗？**
...
**Q22：tokenizer.json 一般有多大？支持中英文的话，要包含所有词语吗？**
...
**Q23：tokenizer.json 和 .safetensors 的关系是什么？每个 token 对应一组向量吗？**
...
- [快手一面：RAG 主要用来解决什么问题？](https://mp.weixin.qq.com/s/kiijFuRNWlRVls2Qaqzwyw) — 小林coding，2026-04-17
